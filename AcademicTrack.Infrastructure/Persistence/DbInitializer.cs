...
public static class DbInitializer
{
    public static void Seed(AcademicTrackDbContext context)
    {
        try
        {
            // Ejecuta automáticamente todas las migraciones pendientes de EF Core al iniciar
            context.Database.Migrate();

            // 1. Sembrado de Programas Académicos
            ...
            // 2. Sembrado de Indicadores base
            if (!context.Indicadores.Any())
            {
                var indicadores = new[]
                {
                    new Indicador { Nombre = "Tasa de deserción", Unidad = "%", Direccion = DireccionIndicador.Descendente },
                    new Indicador { Nombre = "Tasa de aprobación", Unidad = "%", Direccion = DireccionIndicador.Ascendente },
                    new Indicador { Nombre = "Publicaciones indexadas Scopus", Unidad = "artículos", Direccion = DireccionIndicador.Ascendente },
                    new Indicador { Nombre = "Tiempo promedio de graduación", Unidad = "semestres", Direccion = DireccionIndicador.Descendente }
                };
                context.Indicadores.AddRange(indicadores);
                context.SaveChanges();
            }

            // 3. Sembrado de Períodos Académicos
            ...
            // 4. Sembrado de Seguimiento de Cohortes
            ...
            // 5. Sembrado de Seguimiento de Egresados
        ...
        {
            Console.WriteLine($"Error al ejecutar migraciones o sembrar datos: {ex.Message}");
        }
    }
}
